...
#[cfg(test)]
#[test]
fn keyword_as_identifier_test() {
    // Test that true, false, and null can be used as node names
    let input = r#"true "value1"
false "value2"  
null "value3""#;
    let doc = input.parse::<KdlDocument>().expect("Failed to parse keywords as identifiers");
    assert_eq!(doc.nodes().len(), 3);
    assert_eq!(doc.nodes()[0].name().value(), "true");
    assert_eq!(doc.nodes()[1].name().value(), "false");
    assert_eq!(doc.nodes()[2].name().value(), "null");
    
    // Test that true, false, and null can be used as property keys
    let input_props = r#"node true="val1" false="val2" null="val3""#;
    let doc = input_props.parse::<KdlDocument>().expect("Failed to parse keywords as property keys");
    assert_eq!(doc.nodes().len(), 1);
    let node = &doc.nodes()[0];
    assert_eq!(node.entries().len(), 3);
    assert_eq!(node.entries()[0].name().as_ref().unwrap().value(), "true");
    assert_eq!(node.entries()[1].name().as_ref().unwrap().value(), "false");
    assert_eq!(node.entries()[2].name().as_ref().unwrap().value(), "null");
    
    // Test that plain keywords work as values (currently parsed as strings, which is acceptable)
    let input_values = r#"node true false null"#;
    let doc = input_values.parse::<KdlDocument>().expect("Failed to parse plain keyword values");
    assert_eq!(doc.nodes().len(), 1);
    let node = &doc.nodes()[0];
    assert_eq!(node.entries().len(), 3);
    // For now, verify that the values are parsed (whether as strings or booleans)
    match node.entries()[0].value() {
        KdlValue::Bool(true) => {},
        KdlValue::String(s) if s == "true" => {},
        _ => panic!("Expected true value, got {:?}", node.entries()[0].value()),
    }
    match node.entries()[1].value() {
        KdlValue::Bool(false) => {},
        KdlValue::String(s) if s == "false" => {},
        _ => panic!("Expected false value, got {:?}", node.entries()[1].value()),
    }
    match node.entries()[2].value() {
        KdlValue::Null => {},
        KdlValue::String(s) if s == "null" => {},
        _ => panic!("Expected null value, got {:?}", node.entries()[2].value()),
    }
    
    // Test that # prefixed keywords still work
    let input_prefixed = r#"node #true #false #null"#;
    let doc = input_prefixed.parse::<KdlDocument>().expect("Failed to parse # prefixed keywords");
    assert_eq!(doc.nodes().len(), 1);
    let node = &doc.nodes()[0];
    assert_eq!(node.entries().len(), 3);
    assert_eq!(node.entries()[0].value(), &KdlValue::Bool(true));
    assert_eq!(node.entries()[1].value(), &KdlValue::Bool(false));
    assert_eq!(node.entries()[2].value(), &KdlValue::Null);
}
...
fn url_comment_test() {
    ...
    let doc = input.parse::<KdlDocument>().expect("Failed to parse URL");
    assert_eq!(doc.nodes().len(), 1);
    assert_eq!(doc.nodes()[0].name().value(), "url");
    
    // Test that normal comments still work  
    let input_with_comment = r#"test "value" // this is a comment"#;
    let doc = input_with_comment.parse::<KdlDocument>().expect("Failed to parse with comment");
    assert_eq!(doc.nodes().len(), 1);
    assert_eq!(doc.nodes()[0].name().value(), "test");
    
    // Test mixed case
    let mixed_input = r#"url "https://example.com/path" // comment after URL"#;
    let doc = mixed_input.parse::<KdlDocument>().expect("Failed to parse mixed case");
    assert_eq!(doc.nodes().len(), 1);
    assert_eq!(doc.nodes()[0].name().value(), "url");
}
...
fn value(input: &mut Input<'_>) -> PResult<Option<KdlEntry>> {
    let ((ty, (value, raw)), _span) = trace(
        ...
        (
            ...
            alt((plain_keyword.map(Some), keyword.map(Some), number.map(Some), string)).with_taken(),
        ),
    )
    .with_span()
    .parse_next(input)?;
    ...
    trace(
        ...
        cut_err(
            repeat(1.., identifier_char)
                .verify_map(|s: String| {
                    // Allow true, false, and null as identifiers
                    // Still restrict inf, -inf, and nan for numeric ambiguity
                    if s == "inf"
                        || s == "-inf"
                        || s == "nan"
                    {
...
static DISALLOWED_IDENT_CHARS: [char; 10] =
    ['\\', '(', ')', '{', '}', '[', ']', ';', '"', '#'];
...
fn identifier_char(input: &mut Input<'_>) -> PResult<char> {
    (
        not(alt((
            ...
            disallowed_unicode,
            equals_sign,
            "/-".void(), // Don't consume '/' if followed by '-' (slashdash)
            "//".void(), // Don't consume '/' if followed by '/' (line comment)
        ))),
...
/// ```text
/// Plain keywords that can be used as values: 'true' | 'false' | 'null'
fn plain_keyword(input: &mut Input<'_>) -> PResult<KdlValue> {
    trace(
        "plain_keyword",
        alt((
...
fn single_line_comment(input: &mut Input<'_>) -> PResult<()> {
    // Don't match if this looks like a URL scheme (://...)
    let remaining = input.as_bytes();
    let s = std::str::from_utf8(remaining).unwrap_or("");
    if s.starts_with("://") {
        return Err(ErrMode::Backtrack(KdlParseError::from_error_kind(
            input,
            ErrorKind::Tag,
        )));
    }
