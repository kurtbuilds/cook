...
impl PartialEq for KdlEntry {
    fn eq(&self, other: &Self) -> bool {
        self.ty == other.ty && self.value == other.value && self.name == other.name && self.format == other.format
...
impl KdlEntry {
    ...
    pub fn autoformat(&mut self) {
        ...
        if !self.format.as_ref().map(|f| f.autoformat_keep).unwrap_or(false) {
            self.format = None;
        } else {
            #[cfg(feature = "v1")]
            self.ensure_v2();
    ...
    pub fn parse(s: &str) -> Result<Self, KdlError> {
        ...
        {
            v2_parser::try_parse(v2_parser::padded_node_entry, s).or_else(|e| KdlEntry::parse_v1(s).map_err(|_| e))
        }
    }
        ...
        let value_repr = self.format.as_ref().map(|x| {
            match &self.value {
                KdlValue::String(val) => {
                    ...
                    let s = if crate::value::is_plain_ident(val) && !s.starts_with('\"') && !s.starts_with("r#") {
                        format!("\"{val}\"")
                    } else if s
                        .find(|c| v2_parser::NEWLINES.iter().any(|nl| nl.contains(c)))
                        .is_some()
                    {
                        ...
                        if s.contains("\"\"\"") {
                            let prefix = s
                                ...
                                .rev()
                                .skip_while(|c| c == &'"')
                                .take_while(|c| v2_parser::NEWLINES.iter().any(|nl| nl.contains(*c)))
                                .collect::<String>();
